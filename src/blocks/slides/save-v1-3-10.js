...
/**
 * Immutable save function matching the markup emitted by version 1.3.10.
 * Gutenberg needs the historical output to validate and migrate old blocks.
 */
function save( props ) {
	...
	let data_atts = {
		// Version 1.3.10 serialized this string without coercing it.
		slidesPerView,
		slidesPerGroup,
		slidesPerGroupAuto,
		slidesPerGroupSkip,
	...
	data_atts.pagination.type =
		pagination_type != 'bullets' ? pagination_type : 'bullets';
	...
	const thumbsElements = (
		typeof tabsData !== 'undefined' ? tabsData : []
	).map( ( tab, index ) => {
		return (
	...
	function getOverlayImg( image, style ) {
		if ( image === undefined ) {
			return;
		}
		return <div className="wp-swiper__overlay-img" style={ style }></div>;
	}
						...
						<div className="wp_swiper__navigation-container">
							<div
								className={ `swiper-button-prev ${
									previousIcon ? 'wp_swiper__button-prev' : ''
								}` }
							>
								{ previousIcon ? (
							...
							<div
								className={ `swiper-button-next ${
									nextIcon ? 'wp_swiper__button-next' : ''
								}` }
							>
								{ nextIcon ? (
	...
	function getQuoteSVG( { attributes } ) {
		let quoteClassName = attributes.className;

		quoteClassName = quoteClassName ? quoteClassName.toString() : '';

		if ( quoteClassName.indexOf( 'is-style-testimonials' ) !== -1 ) {
			return (
				<>
