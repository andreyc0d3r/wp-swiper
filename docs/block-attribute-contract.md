...
| Editor-only state | `currentSlide`, `tabActive`, `buttonsAlign` | `tabActive` and `buttonsAlign` preserve editor selection and layout. `currentSlide` is retained as an unused legacy field so existing block comments remain valid. |
...
| Autoplay runtime | `autoplay`, `delay`, `disableOnInteraction`, `pauseOnMouseEnter`, `reverseDirection`, `stopOnLastSlide`, `waitForTransition` | Translated into the Swiper autoplay object only when autoplay is enabled. Accessibility controls can pause the resulting instance independently. |
